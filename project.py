...
def OrderItem(orderid, orderqty):
     selectSql = "SELECT * FROM Item WHERE id = %s"
...
def menu_list():
    selectSql = " SELECT * FROM Item WHERE product_qty > '0'"
    ...
    if len(rows) == 0 :
        ...
        orderid = input(" - 상품의 ID : ")
        orderqty = input(" - 주문 개수 : ")
...
def MaxItem(month) : 
    selectSql = """SELECT COUNT(*), MONTH(created_at)as month, item_id, SUM(order_qty)as sum FROM ItemOrder 
    ...
    for row in rows:
        print(row)
    print("{0}월에 가장 많이 팔린 상품은 ITEM_ID : {1}상품 {2}개가 팔렸습니다. ".format(month, rows[0][2], int(rows[0][3])))
...
def UserMenu():
        while True:
            ...
            user_menu = int(input("메뉴를 선택해 주세요. ->"))
            if user_menu == 1 : 
                menu_list()
            elif user_menu == 2 :
                OrderProduct()
